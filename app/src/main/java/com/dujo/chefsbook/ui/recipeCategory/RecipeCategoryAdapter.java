...
public class RecipeCategoryAdapter
    extends ListAdapter<RecipeCategory, RecipeCategoryAdapter.RecipeCategoryVH> {

  private static final DiffUtil.ItemCallback<RecipeCategory> DIFF_CALLBACK =
      new DiffUtil.ItemCallback<RecipeCategory>() {
        @Override
        public boolean areItemsTheSame(
            @NonNull RecipeCategory oldItem, @NonNull RecipeCategory newItem) {
  ...
  public class RecipeCategoryVH extends RecyclerView.ViewHolder {
    ...
    void bind(RecipeCategory recipeCategory) {
      ...
      RequestOptions options =
          new RequestOptions().transform(new CenterCrop(), new RoundedCorners(radiusPx));
